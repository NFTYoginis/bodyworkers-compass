...
> credibility ceiling.** Connect the maps; never collapse them. Never assert that
...
## 1. Per-map evidence-tier table (posterior-chain slice)
...
## 2. Converge / Diverge map — posterior-chain region

**Where the four maps genuinely CONVERGE (shared physical corridor):**

- All four trace the **back-of-body**: sole of foot → calf → hamstring → low back
  → up the spine.
- All four pass through the **lumbar / SI / hip** region implicated in chronic
  low-back complaints.
- The connective-tissue plane underlying the SBL, the Bladder meridian, and Sen
  Kalathari often occupies *the same physical real estate* (Langevin & Yandow's
  location finding). The overlap is **anatomical, not theoretical.**

**Where they genuinely DIVERGE:**

- **Mechanism.** Biomedical = nerve, joint, muscle, fascia. Anatomy Trains =
  mechanical force transmission along fascial continuity. TCM = qi flow along
  meridians. Sen = energy lines in a lineage tradition. These are *different kinds
  of explanation*, not translations of one another.
...
## 3. The governor (connect without collapsing)

Thomas Myers — the author of Anatomy Trains himself — refuses to equate his
myofascial meridians with acupuncture meridians. He calls them **lines of
latitude and longitude**: two different coordinate systems laid over the same
territory. Maps, not the terrain.

That is the governing image of this whole tool. The maps overlap on the terrain.
They are not the same map. The win is never "they're all one" — it's a translator
that knows precisely where the coordinate systems agree, where they don't, and
how much we can trust each.

---

## 4. The tiers, defined (so a tag is never vague)
...
## 5. Anti-claims — the tool must NEVER say these
...
The fuller language bank — what to say *instead* of each — is in
`register-discipline.md`.
...
*Evidence floor produced 2026-05-28 at research-claude's standard (citations
web-verified) and inherited verbatim by this build. The same honesty gate that
validated it runs on the build commit before any public push.*
